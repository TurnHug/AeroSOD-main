...
def check_version(current: str = '0.0.0',
    ...
    """
    current, minimum = (_Ver(x) for x in (current, minimum))
...
def check_pip_update_available():
    ...
    if ONLINE and is_pip_package():
        with contextlib.suppress(Exception):
            ...
            if _Ver(__version__) < _Ver(latest):  # update is available
...
def check_requirements(requirements=ROOT.parent / 'requirements.txt', exclude=(), install=True, cmds=''):
    ...
    if isinstance(requirements, Path):  # requirements.txt file
        ...
        with file.open() as f:
            requirements = [f'{_Req(line.strip()).name}{_Req(line.strip()).specifier}' for line in f if line.strip() and not line.startswith('#') and _Req(line.strip()).name not in exclude]
    ...
    for r in requirements:
        try:
            ...
            _ver(_Req(r).name)
        except (PackageNotFoundError, Exception):  # exception if requirements not met
            try:  # attempt to import (slower but more accurate)
                import importlib
                importlib.import_module(_Req(r).name)
            except ImportError:
                s += f'"{r}" '
                n += 1
